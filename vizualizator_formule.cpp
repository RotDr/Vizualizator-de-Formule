...
void integraladefdim(nod*p)
{
    ...
    nod*c=p->dr->st;
    nod*d=p->dr->dr;
    p->L=font*p->marime+a->L+font*p->marime+b->L+textwidth("d");
    p->H=max(a->centery-a->ymin,b->centery-b->ymin)+max(a->ymax-a->centery,b->ymax-b->centery);
    a->xmin=font*p->marime;
    a->xmax=a->xmin+a->L;
    b->xmin=a->xmax+font*p->marime+textwidth("d");
    b->xmax=b->xmin+b->L;
    if(a->centery-a->ymin>=b->centery-b->ymin)
    {
        a->ymin=0;
    ...
    b->xmin+=max(c->L,d->L);
    a->xmax=a->xmin+a->L;
    b->xmin=a->xmax+textwidth("d")+font*p->marime;
    b->xmax=b->xmin+b->L;
}
